...
class RKNNSiamRPNTracker(SiameseTracker):
    def __init__(self):
    ...
    def exportRKNN(self):
        ...
        if ret != 0:
            ...
            print(f"export {modelExemplarRknnName} Done!")

        # export modelInstanceName
        modelInstanceRknnName = './rknn/modelInstance287.rknn'
        print(f"export {modelInstanceRknnName}")
        input_size_list = [[1, 3, 287, 287]]
        ...
        if ret != 0:
            ...
            print(f"export {modelInstanceRknnName} Done!")
        
        # export modelHeadNameTrace
        modelHeadNameTraceRknnName = './rknn/modelHeadNameTrace.rknn'
        print(f"export {modelHeadNameTraceRknnName}")
        ...
        if ret != 0:
            ...
            print(f"export {modelHeadNameTraceRknnName} Done!")
        
        # # export modelHeadNameScript
        # modelHeadNameScriptRknnName = './rknn/modelHeadNameScript.rknn'
        # print(f"export {modelHeadNameScriptRknnName}")
        # input_size_list = [[1, 256, 6, 6],[1, 256, 26, 26]]

        # # Create RKNN object
        # rknnHeadNameScript = RKNN(verbose=True)

        # # Pre-process config
        # print('--> Config model')
        # rknnHeadNameScript.config(mean_values=[0]*256, std_values=[1]*256, target_platform='rk3588')
        # print('done')

        # # Load model
        # print('--> Loading model')
        # ret = rknnHeadNameScript.load_pytorch(model=modelHeadNameScript, input_size_list=input_size_list)
        # if ret != 0:
        #     print('Load model failed!')
        #     exit(ret)
        # print('done')
        ...
        # ret = rknnHeadNameScript.export_rknn(modelHeadNameScriptRknnName)
        # if ret != 0:
        #     print('Export rknn model failed!')
        #     exit(ret)
        # else:
        #     print(f"export {modelHeadNameScriptRknnName} Done!")

    def initRKNN(self):
        # Init Exemplar runtime environment
        print('--> Init Exemplar runtime environment')
        ret = self.rknnExemplar.init_runtime()
        if ret != 0:
        ...
        ret = self.rknnInstance.init_runtime()
        if ret != 0:
        ...
        ret = self.rknnHeadTrace.init_runtime()
        if ret != 0:
